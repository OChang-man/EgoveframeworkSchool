...
<%--										 Core Tag [c: ]											 --%>
		...
		<c:when test="${step eq '3'}">
			<li>
				<h3>c:import Tag : 일반 변수를 생성해서 값을 할당</h3>
		...
		<c:when test="${step eq '4'}">
			<li>
				...
				<c:if test="${test eq '0'}">      <!-- eq(equal):동등, ne(not equal):비동등-->
					true
				</c:if>
			</li>
		</c:when>
		<c:when test="${step eq '5'}">
			<li>
				<h3>c:choose, c:when, c:ortherwise, Tag : 조건문 중의 하나. JAVA에서 사용하는 If, else If, else와 비슷 함</h3>
				<h4>기본문법 : <br/>
					&#60;c:choose&#62;<br/>
						&#60;c:when test="조건문"&#62; 조건 true에 대한 내용 &#60;c:when&#62;<br/>
						&#60;c:otherwise&#62;조건에 해당되지 않아서 나오는 내용 &#60;c:ohterwise&#62;<br/>
					&#60;c:choose&#62;
				</h4>
		...
		<c:when test="${step eq '6'}">
			<li>
				<h3>c:forEach Tag : 기본 반복 태그, 고정 된 횟수 또는 초과 수집 동안 중첩 된 본문 내용을 반봉</h3>
				<h4>기본문법 : <br/>
				...
				<c:forEach var="result" items="${fn:split(str,',')}">
		...
		<c:when test="${step eq '7'}">
			<li>
				<h3>c:param Tag : 포함하는 import, url 테그의 URL에 매개 변수를 추가</h3>
		...
		<c:when test="${step eq '8'}">
			<li>
				...
				ULR 1 : <c:url var="link1" value="https://www.naver.com"/> <br/>
				ULR 1 : <c:url var="link2" value="/temp/jstlImport.do">
							<c:param name="test" value="테스트 호출"/>
						</c:url>
						<br/>
...
<%--										 Function Tag [fn: ]											 --%>		
		...
		<c:when test="${step eq '10'}">
			<li>
				<h3>fn:indexOf() : 주어진 문자열이 특정 문자열을 포함하면 주어진 문자열 내에서 인데스를 반환</h3>
